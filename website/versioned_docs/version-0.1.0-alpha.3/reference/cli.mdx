...
| `--invk-config` | `-c` | Config file path (default: `$HOME/.config/invowk/config.cue`) |
| `--invk-interactive` | `-i` | Run commands in alternate screen buffer (interactive mode) |
...
| Flag | Short | Description |
|------|-------|-------------|
| `--invk-runtime` | `-r` | Override the runtime (must be allowed by the command) |
| `--invk-from` | | Source to run command from (e.g., 'invkfile' or module name) |

Built-in per-command flags:

| Flag | Short | Description |
|------|-------|-------------|
| `--invk-env-file` | `-e` | Load environment variables from file (repeatable) |
| `--invk-env-var` | `-E` | Set environment variable (KEY=VALUE, repeatable) |
| `--invk-env-inherit-mode` |  | Inherit host environment variables: `none`, `allow`, `all` |
| `--invk-env-inherit-allow` |  | Allowlist host env vars to inherit (repeatable) |
| `--invk-env-inherit-deny` |  | Denylist host env vars to inherit (repeatable) |
| `--invk-workdir` | `-w` | Override working directory for this command |

:::note
The `invk-`, `invowk-`, and `i-` prefixes are reserved for system flags and cannot be used for user-defined flags.
:::
...
:::tip
You can use `@source` or `--invk-from` even for unique command names to be explicit about where a command comes from.
:::
...
- Run with verbose mode: `invowk --invk-verbose cmd`
