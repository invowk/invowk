...
:::tip
Unlike `invowk module validate`, `invowk validate` always performs deep validation
(including invowkfile parsing and command tree checks) without requiring a `--deep` flag.
:::
