...
Default LLM backend configuration. The whole `llm` object is unset by `DefaultConfig()` until the user configures a provider or API endpoint.
...
**Required:** No
**Default:** `{enabled: true, strict: false, inherit_includes: true}`
