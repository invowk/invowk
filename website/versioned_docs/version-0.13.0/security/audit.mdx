...
**Detects:**
- Risky `env_inherit_mode: "all"` and unset native/virtual runtime `env_inherit_mode` defaults (both expose all host environment variables)
...
The LLM checker works with OpenAI-compatible servers that provide chat completions and model listing, because invowk verifies the configured model before scanning. The default configuration targets [Ollama](https://ollama.com), the most popular local LLM server:
