...
- **Published under a command source ID** derived from the module URL/path or alias to prevent collisions
...
When a dependency module is installed in a discovery path, its commands publish under a command source ID. The default source ID is derived from the module URL or path; `alias` overrides it. Run the simple command name when unique, or use `@<source>` / `--ivk-from <source>` to disambiguate:
