...
Root, command, and implementation-level filepath checks are always validated on the host. To validate paths inside a container image, put `depends_on` inside the selected container runtime block:
