...
:::note
Interactive mode works with **all runtimes**: native, virtual-sh, virtual-lua, and container.
:::
...
Interactive mode works seamlessly with the container runtime. Container runs already use the auto-provisioned image layer (enabled by default) so `invowk` is available inside the container; interactive mode adds the TUI plumbing. See [Container Runtime](/docs/runtime-modes/container#auto-provisioning) for details. When you run a container command with `-i`:
...
<Snippet id="interactive/container-tui-example" />

:::warning Linux Containers Only
Alpine-based images are not supported — musl-based environments have subtle behavioral differences that reduce runtime reliability. Use `debian:stable-slim` for generic container examples; language-specific images belong only in language-specific runtime demonstrations.
:::
...
<Snippet id="interactive/deploy-example" />

Run with: `invowk cmd myproject deploy -i`

### Database Migration

<Snippet id="interactive/db-migrate-example" />
