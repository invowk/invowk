...
This diagram zooms into the **Container Engine Abstraction** container from the [C2 Container Diagram](./c4-container) to show the internal components of the `internal/container` package and the adjacent runtime, planning, and provisioning ports that consume them. It reveals how Invowk provides a unified interface over Docker and Podman CLIs, handles Podman-specific concerns (SELinux, rootless), and transparently adapts to sandboxed environments (Flatpak, Snap).
...
| **VolumeFormatFunc** | Transforms `VolumeMountSpec` values before passing them to `-v`. Podman uses this to append SELinux labels (`:z`) on Linux. |
