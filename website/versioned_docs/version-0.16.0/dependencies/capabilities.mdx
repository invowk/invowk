...
|------------|-------------|
| `local-area-network` | Detects an active non-loopback interface with a routable IP address |
...
Check for LAN availability:

<Snippet id="dependencies/capabilities-lan" />

This checks:
- An active, non-loopback network interface exists
- The interface has a routable IP address

It does not send traffic or prove that a particular LAN service is reachable.
...
For container runtime, root, command, and implementation capability checks run on the **host**. Capability checks declared inside the selected container runtime configuration run **inside the container**:

<Snippet id="dependencies/capabilities-container-context" />

Use host checks for deployment reachability and other workstation requirements. Use runtime-level container checks when the command needs the container image itself to prove that a service is reachable or a capability is available from inside the execution environment.
