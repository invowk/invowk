...
If you use [Homebrew](https://brew.sh/) on **macOS** (Invowk is distributed as a Homebrew cask, which Homebrew on Linux does not support):
...
Release artifacts are signed with [Cosign](https://github.com/sigstore/cosign) (keyless, via GitHub Actions OIDC). To verify the checksums file downloaded from a GitHub Release:
...
Then verify individual archives against `checksums.txt`:
