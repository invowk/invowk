...
- **Windows reserved filenames**: Files named `CON`, `PRN`, `AUX`, `NUL`, `COM1`-`COM9`, or `LPT1`-`LPT9` create a compatibility validation issue. The module is invalid and `invowk validate` exits with status 1 because these names cannot be extracted reliably on Windows.
