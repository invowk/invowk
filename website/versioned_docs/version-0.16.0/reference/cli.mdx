...
| `--ivk-config` | `-c` | Config file path (default: [OS-specific config directory](/docs/configuration/overview#configuration-file-location)) |
...
| `--ivk-container-name` | | Override the persistent container target name (container runtime only) |
...
| `--ivk-env-inherit-allow` |  | Allowlist host env vars to inherit; requires `--ivk-env-inherit-mode=allow` (repeatable) |
...
- `default` - A `hello` command demonstrating the platform-split pattern across the built-in runtime options
- `minimal` - Bare minimum invowkfile structure
- `full` - Template demonstrating subcommands, flags, environment configuration, and command dependencies
...
| Flag | Description |
|------|-------------|
| `--alias` | Alias for the module command source ID |
...
Add missing transitive dependencies to `invowkmod.cue`. Invowk uses an explicit-only dependency model (like Go modules): every module in the dependency tree must be declared in your root `invowkmod.cue`. This command scans resolved modules and auto-adds any missing transitive requirements.
...
| Path | Validation |
|------|-----------|
| `*.invowkmod` directory | Module validation (structure, plus invowkfile parsing and command tree when `invowkfile.cue` is present) |
...
:::tip
`invowk validate` performs deep validation on module structure and metadata. When the
module contains `invowkfile.cue`, it also parses that file and validates its command tree;
library-only modules do not run those file-specific checks.
:::
...
Scan for security risks in invowkfiles, modules, and scripts. See the [Security Auditing](../security/audit) guide for full documentation.
...
| `--llm-provider` | | Auto-detect or force provider: `auto`, `claude`, `codex`, `gemini`, `ollama` |
| `--llm` | `false` | Enable LLM using configured or API settings |
| `--llm-url` | `http://localhost:11434/v1` | OpenAI-compatible API base URL |
| `--llm-model` | API/Ollama fallback: `qwen2.5-coder:7b`; CLI provider tool default when omitted | Model name; required for cloud API providers and optional for CLI providers |
| `--llm-api-key` | *(empty)* | API key (empty for local servers) |
| `--llm-timeout` | `2m` | Per-request timeout |
| `--llm-concurrency` | `2` | Max parallel LLM requests |

`--llm` and `--llm-provider` are mutually exclusive. Use `--llm` for the configured/API backend and `--llm-provider` to force provider detection.
...
Print operation-aware system prompts that LLM agents should use when authoring custom Invowk commands.

<Snippet id="reference/cli/agent-prompt-syntax" />

The optional `operation` is `create`, `change`, or `remove`. Omit it to print all three operation prompts.
...
Generate one custom command with an LLM, validate it, and patch `invowkfile.cue`. The generated command `name` must match `<name>`. See [LLM-Assisted Agent Authoring](../advanced/llm-assisted-authoring) for the full workflow.
...
| Flag | Default | Description |
|------|---------|-------------|
| `--file` | `invowkfile.cue` | Invowkfile to update |
| `--from-file` | | Read the command request from a file |
| `--dry-run` | `false` | Print the patch without writing |
| `--print` | `false` | Print only the generated command CUE |
| `--verify` | `false` | After writing, resolve the generated command with a dry-run execution plan |
| `--llm-provider` | | Auto-detect or force provider: `auto`, `claude`, `codex`, `gemini`, `ollama` |
| `--llm` | `false` | Enable LLM using configured or API settings |
| `--llm-url` | `http://localhost:11434/v1` | OpenAI-compatible API base URL |
| `--llm-model` | API/Ollama fallback: `qwen2.5-coder:7b`; CLI provider tool default when omitted | Model name; required for cloud API providers and optional for CLI providers |
...
Generate a replacement for one existing custom command with an LLM. The generated command `name` must still match `<name>`, and only that command is replaced.

<Snippet id="reference/cli/agent-change-syntax" />

**Flags:** same as `agent cmd create`.

### invowk agent cmd remove

Remove one custom command from an invowkfile without resolving an LLM provider.
...
Print operation-aware system prompts that LLM agents should use when authoring local Invowk modules.
...
Generate `invowkmod.cue` and `invowkfile.cue` for `<module-id>.invowkmod` with an LLM. The generated module ID must match `<module-id>`.
...
| Flag | Default | Description |
|------|---------|-------------|
| `--from-file` | | Read the module request from a file |
| `--dry-run` | `false` | Print the file patches without writing |
| `--print` | `false` | Print the generated module file bundle without writing |
| `--verify` | `false` | After writing, validate the generated module |
| `--scripts` | `false` | Create an empty `scripts/` directory with `.gitkeep` |
| `--llm-provider` | | Auto-detect or force provider: `auto`, `claude`, `codex`, `gemini`, `ollama` |
| `--llm` | `false` | Enable LLM using configured or API settings |
| `--llm-url` | `http://localhost:11434/v1` | OpenAI-compatible API base URL |
| `--llm-model` | API/Ollama fallback: `qwen2.5-coder:7b`; CLI provider tool default when omitted | Model name; required for cloud API providers and optional for CLI providers |
...
Generate replacements for `invowkmod.cue` and `invowkfile.cue` in an existing local module. No other module files are generated or edited.

<Snippet id="reference/cli/agent-mod-change-syntax" />

**Flags:** same as `agent mod create`, except `--scripts` is not supported.

### invowk agent mod remove

Remove one validated local module directory without editing dependencies or lock files. Deletion requires `--force`; use `--dry-run` to preview the deletion plan.
...
| Flag | Default | Description |
|------|---------|-------------|
| `--dry-run` | `false` | Print the deletion plan without removing the module |
| `--force` | `false` | Delete the validated module directory |

Provide either description arguments or `--from-file`; they are mutually exclusive. `--llm` and `--llm-provider` are mutually exclusive. `--dry-run` and `--print` are mutually exclusive. `--verify` requires writing the invowkfile and cannot be combined with `--dry-run` or `--print`. Verification currently supports only the current directory target (`./invowkfile.cue`); combining `--verify` with `--file` for another path fails before the dry-run execution plan is resolved.
