...
The `script` field contains the commands to execute. Use `script.content` for inline commands. Inside invowk modules, `script.file` can reference a script file contained in that same module.
...
Inside an invowk module, reference a module-contained script file instead of inline code:

<Snippet id="core-concepts/script-external" />

`script.file` is explicit, not extension-detected: extensionless files are valid, and root/project invowkfiles should call local scripts through `script.content`.

### When to Use External Scripts

- **Inline**: Quick, simple commands; keeps everything in one file
- **External**: Complex module-bundled scripts; reusable across module commands; easier to edit with syntax highlighting
...
| Linux | `invowk cmd myproject build --ivk-runtime virtual-sh` | First impl, virtual-sh runtime |
| Windows | `invowk cmd myproject build` | Second impl, native runtime |
| Windows | `invowk cmd myproject build --ivk-runtime virtual-sh` | Error: no matching impl |
...
- `[native*, virtual-sh]` - Supports native (default) and virtual-sh runtimes
...
- [Runtime Modes](../runtime-modes/overview) - Deep dive into native, virtual-sh, virtual-lua, and container runtimes
