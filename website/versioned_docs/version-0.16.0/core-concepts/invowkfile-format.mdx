...
- **Type safety** - Invowk™'s schema ensures your invowkfile is correct
...
Scripts can be inline. Inside invowk modules, they can also reference files contained in the same module:
...
`script.file` is module-only and module-contained. It is not extension-detected; extensionless script files are valid. In root/project invowkfiles, invoke local script files through `script.content`.
