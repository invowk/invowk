...
Enables [u-root](https://github.com/u-root/u-root) utilities in `virtual-sh` and command helpers in `virtual-lua`. When enabled, 28 additional POSIX-compliant commands become available to `virtual-sh`:
...
Configures a default LLM backend for LLM-aware commands. `invowk agent cmd create|change` and `invowk agent mod create|change` use this config automatically when no LLM flags are passed. `invowk audit` still requires explicit opt-in with `--llm` or `--llm-provider` before any script content is sent to an LLM.

Use a supported provider harness:

<Snippet id="config/llm-provider" />

Or use an OpenAI-compatible API endpoint:

<Snippet id="config/llm-api" />
...
**Type:** Go duration string (`#LLMTimeoutDurationString`, max 64 runes)
...
If `api` is present, set at least one of those fields.
