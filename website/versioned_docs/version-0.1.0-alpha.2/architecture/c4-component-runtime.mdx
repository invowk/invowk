...
| **EnvContext** | Groups environment configuration: `ExtraEnv` (INVOWK_FLAG_\*, INVOWK_ARG_\*), `RuntimeEnvVars` (--env-var), `RuntimeEnvFiles` (--env-file), and inheritance overrides. |
...
| 9 | `--env-file` flag |
| 10 | `--env-var` flag (highest priority) |
