...
Packs are self-contained folders that bundle `invkpack.cue` metadata with optional `invkfile.cue` commands and any script files they reference. They're perfect for sharing commands, creating reusable toolkits, and distributing automation across teams.
...
- **Self-contained**: Scripts are bundled with the pack
...
- **`invkpack.cue`**: Pack metadata (name, version, dependencies)

### Optional Contents

- **`invkfile.cue`**: Command definitions (omit for library-only packs)
...
- [Pack Dependencies](./dependencies/overview) - Use other packs in your pack
