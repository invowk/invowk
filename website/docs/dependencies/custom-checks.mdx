...
| `script.content` | Yes | Inline script content to execute for the check |
| `script.file` | Module only | Script file contained in the source invowkmod |
| `script.interpreter` | No | Interpreter for the resolved script content; defaults to shebang detection and then the embedded mvdan/sh shell for host checks |
...
Host custom checks without a shebang or `script.interpreter` run through Invowk's embedded mvdan/sh shell. A non-shell shebang or `script.interpreter` such as `python3` runs through that host interpreter. Container custom checks use the same `script.interpreter` field inside the selected container.
