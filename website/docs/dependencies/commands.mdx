...
- **Modules**: use an explicit source-qualified ref (e.g., `"@shared generate-types"`).
...
If a command dependency is declared in `requires` but still fails the access check, run `invowk module sync` so the lock file records the dependency's stable module ID and command source namespace. If it still fails, the discovered copy differs from the locked content: align the dependency's version with the one your module locked.
