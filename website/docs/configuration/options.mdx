...
**File Operations (13):** `base64`, `cat`, `cp`, `find`, `gzip`, `ln`, `ls`, `mkdir`, `mktemp`, `mv`, `rm`, `tar`, `touch`

**Text Processing (11):** `basename`, `cut`, `dirname`, `grep`, `head`, `realpath`, `sort`, `tail`, `tr`, `uniq`, `wc`
