...
Additional invkfiles and modules to include in command discovery. Each entry is an `#IncludeEntry` with a required `path` and optional `alias`.

<Snippet id="config/includes" />

**Path requirements:**
- Must end with `.invkmod`, `/invkfile.cue`, or `/invkfile`
- Duplicate paths are rejected

**Alias rules:**
- Only valid for `.invkmod` paths
- Must be unique across all includes entries

**Discovery Order:**
1. Current directory (always searched first, highest priority)
2. `~/.invowk/cmds` (always included)
