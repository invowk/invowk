...
The virtual runtime is **not a security sandbox**. It is a portable shell interpreter augmented with built-in utilities. Any command not provided by the built-ins (e.g., `git`, `curl`, `python`, `node`) is resolved from the host system's `PATH` and executed as a native process with full host access. By default, the host's environment variables are also inherited (`env_inherit_mode: "all"`).
...
The virtual runtime **cannot** use non-shell `script.interpreter` values:
...
- **No shell dependency**: Works even if system shell is unavailable (the interpreter is built-in, but external commands still require host binaries)
