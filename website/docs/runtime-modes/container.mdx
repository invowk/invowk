...
:::info Alpine Detection Scope

Invowk rejects images with exact name `alpine` or ending with `/alpine` (after stripping tags and digests). Images with "alpine" elsewhere in the name (e.g., `go-alpine-builder` or `myorg/alpine-tools`) are **not** automatically rejected. Avoid all musl-based images regardless of naming — the policy is about runtime reliability, not just image name matching.
...
You must specify exactly one source, either an `image` or a `containerfile`; they are mutually exclusive.

### Pre-built Images

<Snippet id="runtime-modes/container-prebuilt-image" />

Recommended generic image:
- `debian:stable-slim` - Minimal Debian base for portable container examples
...
Containerfile paths are relative to the invowkfile directory. Parent-directory segments (`..`) are rejected before path resolution, while ordinary dotted names such as `Containerfile..backup` remain valid.
...
By default, Invowk starts an ephemeral container for each command and removes it after the process exits. Set `persistent` on a container runtime block when a command should reuse a long-lived container across invocations:

<Snippet id="runtime-modes/container-persistent-managed" />

When `create_if_missing` is `true`, Invowk creates the container if it does not exist, labels it as Invowk-managed, starts it, and executes the command with `docker exec` or `podman exec` on later runs. If `name` is omitted, Invowk derives a stable name that starts with `invowk-` from the fully-qualified command namespace.

You can also target a pre-existing running container from the CLI:

<Snippet id="runtime-modes/container-persistent-cli-target" />

Explicit names must be portable Docker/Podman names: lowercase letters, digits, `.`, `_`, or `-`, starting with a lowercase letter or digit.
...
- Tokens are revoked when the command completes, which also closes any SSH connection opened with them
...
Auto-provisioning applies to image-based container executions, including both non-interactive and interactive runs. When a command targets an existing persistent container, Invowk skips image preparation and uses that container directly. For image-based runs, the derived image is cached as `invowk-provisioned:<hash>` and reused; if provisioning fails, Invowk warns and runs the base image only in the default non-strict mode. With `container.auto_provision.strict: true`, provisioning failure stops the command.
...
docker image inspect debian:stable-slim
```
...
```bash
# Pre-pull the recommended generic image
...
- [Virtual-Sh Runtime](./virtual) - For cross-platform scripts
