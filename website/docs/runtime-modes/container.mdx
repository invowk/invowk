...
**NOT supported:**
- **Alpine-based images** (`alpine:*`, `*-alpine`) — BusyBox's `ash` shell has incompatibilities
...
Scripts are executed using `/bin/sh` inside the container, which requires a glibc-based Linux environment.
...
Common images:
- `debian:bookworm-slim` - Minimal Debian
- `ubuntu:22.04` - Full Ubuntu
- `golang:1.21` - Go development
- `node:20` - Node.js development
- `python:3.11-slim` - Python development

:::warning glibc Requirement
The invowk binary is dynamically linked against **glibc**. Container images using musl libc (like Alpine Linux) are **not compatible**. Use glibc-based images such as:
- Debian/Ubuntu-based images (default for most official images)
- Images with `-slim` suffix (e.g., `python:3.11-slim`, `node:20-slim`)
- Official language images without `-alpine` suffix

**Not supported:** `alpine`, `busybox`, images with `-alpine` suffix
:::
