...
This diagram zooms into the **Container Engine Abstraction** container from the [C2 Container Diagram](./c4-container) to show the internal components of the `internal/container` package and the adjacent runtime, planning, and provisioning ports that consume them. It reveals how Invowk provides a unified interface over Docker and Podman CLIs, applies SELinux volume labels for both engines, handles Podman-specific rootless concerns, and transparently adapts to sandboxed environments (Flatpak, Snap).
...
|-----------|------------|----------------|
| **Engine** | Go interface | 12-method lifecycle and execution contract: `Name`, `Available`, `Version`, `Build`, `Run`, `InspectContainer`, `Create`, `Start`, `Exec`, `Remove`, `ImageExists`, `RemoveImage`. All factory functions return this type. |
| **CommandPreparer** | Go interface | Interactive adapter contract: `BinaryPath`, `BuildRunArgs`, `PrepareRunCommand`. Kept separate from `Engine`. |
...
| Component | Technology | Responsibility |
|-----------|------------|----------------|
| **BaseCLIEngine** | Go struct | Shared base for CLI-based engines. Provides argument builders (`BuildArgs`, `RunArgs`, `ExecArgs`, `CreateArgs`, `StartArgs`) plus inspect/create/start/exec operations, command preparation, and lifecycle coordination. Configured via functional options. |
| **DockerEngine** | Go struct | Embeds `*BaseCLIEngine`. Implements `Engine`. Locates the `docker` binary via `exec.LookPath`. Injects SELinux volume labels (`:z`) when the daemon reports SELinux in `docker info`. |
...
| **VolumeFormatFunc** | Transforms `VolumeMountSpec` values before passing them to `-v`. Docker and Podman use this to append SELinux labels (`:z`). |
| **RunArgsTransformer** | Post-processes run arguments. Podman uses this to inject `--userns=keep-id` for rootless compatibility. |
| **SELinuxCheckFunc** | Determines whether SELinux labeling should be applied. Injected into both engines' volume formatters: Podman checks the local host, Docker asks its daemon. |
...
| **CreateOptions** | Input for `Engine.Create()`: image, idle command, work dir, env, labels, volumes, ports, name, extra hosts. |
| **CreateResult** | Output from `Engine.Create()`: container ID. |
| **RunResult** | Output from `Engine.Run()`: container ID, exit code, error. |
| **ContainerInfo** | Output from `Engine.InspectContainer()`: container ID, name, running state, status, labels. |
...
|--------------------|----------|--------------|
| **runtime.ContainerRuntime** | Private `runtime.containerEngine` port | Stores the narrowed engine contract needed for image build, one-shot runs, and persistent inspect/create/start/exec operations. The Docker, Podman, and sandbox-aware engine implementations satisfy this port. |
| **provision.Provisioner** | Image-provisioning port stored by `ContainerRuntime` | `LayerProvisioner` is the production implementation created by the runtime constructors. |
...
| **runtime.persistentContainerPlan** | `containerplan.ResolvePersistentTarget` | Delegates the ephemeral-versus-persistent target decision to `internal/containerplan`; `ContainerRuntime` then applies the returned `PersistentPlan` through engine lifecycle operations. |
...
- **Engine-specific behavior**: both constructors prepend `WithVolumeFormatter` (SELinux); Podman's also prepends `WithRunArgsTransformer` (rootless userns). Both go before user-supplied options.
