...
| 3 | Configured includes | `/opt/invowk-modules/tools.invkmod` |
| 4 (lowest) | User commands directory | `~/.invowk/cmds/mytools.invkmod/` |
...
- The user commands directory (`~/.invowk/cmds/`) only discovers `*.invkmod` directories as immediate children (non-recursive). Loose invkfiles are not discovered from this location.
