---
sidebar_position: 8
---

import Diagram from '@site/src/components/Diagram';
import Snippet from '@site/src/components/Snippet';

# Discovery Precedence Flowchart

This diagram shows how commands are discovered and how conflicts are resolved when the same command name appears in multiple locations.

## Discovery Flow

<Diagram id="architecture/discovery-flow" />

## Conflict Resolution

When the same command name exists in multiple locations:

<Diagram id="architecture/discovery-conflict" />

### Resolution Rules

| Priority | Source | Example Path |
|----------|--------|--------------|
| 1 (highest) | Current directory invowkfile | `./invowkfile.cue` |
| 2 | Local modules | `./mytools.invowkmod/` |
| 3 | Configured includes | `/opt/invowk-modules/tools.invowkmod` |
| 4 (lowest) | User commands directory | `~/.invowk/cmds/mytools.invowkmod/` |

**Key principles**:
- Closer to the working directory = higher priority.
- Configured includes take precedence over the user commands directory because they represent explicit, intentional configuration.
- The user commands directory (`~/.invowk/cmds/`) only discovers `*.invowkmod` directories as immediate children (non-recursive). Loose invowkfiles are not discovered from this location.

## Module Discovery Details

<Diagram id="architecture/discovery-module-structure" />

### Required Module Fields

<Snippet id="architecture/discovery-module-fields" />

## Dependency Resolution

<Diagram id="architecture/discovery-deps" />

### Transitive Dependency Visibility

| From | Can Access | Cannot Access |
|------|------------|---------------|
| Module A | A's commands, B's commands, globally installed modules (`~/.invowk/cmds/`) | C's commands (transitive) |
| Module B | B's commands, C's commands, globally installed modules (`~/.invowk/cmds/`) | - |
| Root invowkfile | Own commands, direct deps, globally installed modules (`~/.invowk/cmds/`) | Transitive deps |

**Why this restriction?**
- Prevents implicit coupling to transitive dependencies
- Makes dependencies explicit in each module
- Enables dependency upgrades without breaking consumers

:::note Static analysis, not runtime interception
This visibility rule is enforced as a **static analysis gate** at `depends_on.cmds` validation time (via `CheckCommandDependenciesExist()` in `internal/app/deps/`). It checks declared inter-module command references — it does **not** intercept subprocess calls. A script that dynamically invokes `invowk cmd ...` at runtime is outside this check. For execution isolation, use the **container** runtime.
:::

## Includes Configuration

Additional modules are configured in `~/.config/invowk/config.cue`:

<Snippet id="architecture/discovery-includes-config" />

### Entry Processing Order

<Diagram id="architecture/discovery-includes" />

## Discovery Caching

<Diagram id="architecture/discovery-cache" />

## Common Discovery Issues

### Problem: Command Not Found

<Diagram id="architecture/discovery-not-found" />

### Problem: Wrong Command Version

<Diagram id="architecture/discovery-wrong-version" />

## Debug Commands

```bash
# List all discovered commands with sources
invowk cmd --ivk-verbose

# Validate workspace discovery (reports all diagnostics)
invowk validate

# Validate a specific module
invowk validate ./mymodule.invowkmod
```

## Related Diagrams

- [Command Execution Sequence](./execution-flow) - What happens after discovery
- [Runtime Selection Flowchart](./runtime-selection) - How runtimes are chosen
- [C4 Container Diagram](./c4-container) - Discovery component context
