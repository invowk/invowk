...
- `invk-env-file` / `-e` - Load environment from file
- `invk-env-var` / `-E` - Set environment variable
- `invk-env-inherit-mode` - Inherit host environment variables
- `invk-env-inherit-allow` - Allowlist host env vars to inherit
- `invk-env-inherit-deny` - Denylist host env vars to inherit
- `invk-workdir` / `-w` - Override working directory
- `help` / `-h` - Show help
- `invk-runtime` / `-r` - Override runtime
- `invk-from` - Override container image
- `invk-force-rebuild` - Force container rebuild

Additionally, the `invk-`, `invowk-`, and `i-` prefixes are reserved for system flags. Don't use these prefixes or names for your flags.
