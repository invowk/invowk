...
Default LLM backend configuration. `DefaultConfig()` derives this from CUE and produces an empty no-backend `llm: {}` object until the user configures a provider or API endpoint.
