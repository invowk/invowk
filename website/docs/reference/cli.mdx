...
Print the system prompt that LLM agents should use when authoring a custom Invowk command.
...
Generate one custom command with an LLM, validate it, and patch `invowkfile.cue`. See [LLM-Assisted Command Authoring](../advanced/llm-assisted-authoring) for the full workflow.
...
| `--replace` | `false` | Replace an existing command with the same name |
