...
| `--llm-model` | `qwen2.5-coder:7b` | LLM model name |
...
`--dry-run` and `--print` are mutually exclusive. `--verify` requires writing the invowkfile and cannot be combined with `--dry-run` or `--print`.
