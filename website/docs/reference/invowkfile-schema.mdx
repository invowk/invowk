...
Dependencies that must be satisfied before running this implementation. Combined with command-level depends_on. Dependencies are validated according to the runtime (native checks the host shell, virtual checks the built-in sh interpreter, container checks the container's shell). See [DependsOn](#dependson).
