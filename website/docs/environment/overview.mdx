...
1. **CLI env vars** - `--ivk-env-var KEY=value`
2. **CLI env files** - `--ivk-env-file .env.custom`
...
| `INVOWK_SOURCE` | Source origin (e.g., module name) | No — only when running from a module |
| `INVOWK_PLATFORM` | Resolved platform (`linux`, `macos`, `windows`) | No — only when running from a module |
