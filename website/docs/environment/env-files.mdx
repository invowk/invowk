...
Variables in `vars` always override values from files. All files are loaded first (root → command → implementation), then all vars are applied on top. See [Precedence](./precedence) for the full order.
