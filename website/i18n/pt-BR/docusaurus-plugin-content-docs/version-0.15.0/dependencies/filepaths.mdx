...
Verificações de caminho no nível raiz, de comando e de implementação são sempre validadas no host. Para validar caminhos dentro de uma imagem de container, coloque `depends_on` dentro do bloco do runtime container selecionado:
