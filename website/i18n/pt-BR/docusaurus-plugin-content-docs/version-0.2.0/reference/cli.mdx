...
:::tip
Diferente de `invowk module validate`, `invowk validate` sempre executa validação profunda
(incluindo parsing de invowkfile e verificações da árvore de comandos) sem necessitar da flag `--deep`.
:::
