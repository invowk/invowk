...
Para análise semântica mais profunda além da correspondência de padrões, habilite a auditoria com LLM usando `--llm-provider` ou a configuração manual `--llm`. Isso envia conteúdo de scripts para uma ferramenta CLI local ou para uma API local/remota compatível com OpenAI.
...
| `--llm` | `false` | &mdash; | Habilitar análise com LLM |
| `--llm-url` | `http://localhost:11434/v1` | `INVOWK_LLM_URL` | URL base da API |
| `--llm-model` | `qwen2.5-coder:7b` para manual/Ollama; padrão da ferramenta para provedores CLI | `INVOWK_LLM_MODEL` para `--llm` manual | Nome do modelo; obrigatório para provedores de API cloud via env var |
