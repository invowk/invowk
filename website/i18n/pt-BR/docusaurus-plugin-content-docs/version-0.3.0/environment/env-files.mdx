...
Variáveis em `vars` sempre sobrescrevem valores de arquivos. Todos os arquivos são carregados primeiro (raiz → comando → implementação), depois todas as vars são aplicadas por cima. Veja [Precedência](./precedence) para a ordem completa.
