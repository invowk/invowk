...
Invkfiles e módulos adicionais para incluir na descoberta de comandos. Cada entrada é um `#IncludeEntry` com `path` obrigatório e `alias` opcional.

<Snippet id="config/includes" />

**Requisitos de caminho:**
- Deve terminar com `.invkmod`, `/invkfile.cue`, ou `/invkfile`
- Caminhos duplicados são rejeitados

**Regras de alias:**
- Válido apenas para caminhos `.invkmod`
- Deve ser único entre todas as entradas includes

**Ordem de Descoberta:**
1. Diretório atual (sempre buscado primeiro, maior prioridade)
2. `~/.invowk/cmds` (sempre incluído)
