...
O template padrão inclui um único comando `hello` com quatro implementações: native (Linux/macOS), native (Windows), virtual (todas as plataformas) e container (somente Linux). Cada implementação declara quais plataformas ela suporta.
