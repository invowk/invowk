...
Instale usando [WinGet](https://learn.microsoft.com/en-us/windows/package-manager/) no **Windows 10/11**:
...
Se você usa [Homebrew](https://brew.sh/) no **macOS** (o Invowk é distribuído como um cask do Homebrew, que não é suportado pelo Homebrew no Linux):
...
### Verificando Assinaturas de Release

Os artefatos de release são assinados com [Cosign](https://github.com/sigstore/cosign) (sem chave, via OIDC do GitHub Actions). Para verificar o arquivo de checksums baixado de um GitHub Release:
...
Em seguida, verifique os arquivos individuais contra o `checksums.txt`:
