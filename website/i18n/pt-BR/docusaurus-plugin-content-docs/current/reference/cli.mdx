...
1. Diretório atual (invowkfile e módulos)
2. `~/.invowk/cmds/` (invowkfiles e módulos)
3. Entradas configuradas em `includes` (invowkfiles e módulos)
...
- Nenhum invowkfile existe em `~/.invowk/cmds/`
- Nenhum invowkfile existe nos `includes` configurados

**Soluções:**
- Execute `invowk init` para criar um invowkfile no diretório atual
- Navegue para um diretório que contenha um invowkfile
- Adicione caminhos de invowkfile ou módulo em `includes` na sua configuração
