...
**Tipo:** mapa de nomes lógicos em maiúsculas para caminho string ou caminhos por plataforma
**Obrigatório:** Não

Caminhos lógicos expostos aos runtimes virtuais. As chaves devem ser sufixos de ambiente seguros, como `CACHE` ou `DB_ROOT`; o Invowk expõe cada mapeamento como `INVOWK_PATH_<KEY>` e o resolve por `invowk.path("<KEY>/file")` no virtual-lua. Os valores podem ser uma string comum ou um mapa com entradas `linux`, `macos` e `windows` para as plataformas selecionadas.
