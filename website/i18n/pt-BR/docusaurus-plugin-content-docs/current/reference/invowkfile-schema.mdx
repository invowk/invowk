...
- `env-file` (curta `e`), `env-var` (curta `E`)
- `env-inherit-mode`, `env-inherit-allow`, `env-inherit-deny`
- `workdir` (curta `w`), `help` (curta `h`), `runtime` (curta `r`)
- `from` (curta `f`), `force-rebuild`, `version`
- Os prefixos `ivk-`, `invowk-` e `i-` também são reservados para flags do sistema
:::
...
Dependências que devem ser satisfeitas antes de executar esta implementation. Combinado com o depends_on no nível de comando. As dependências são validadas de acordo com o runtime (native verifica o shell do host, virtual verifica o interpretador sh embutido, container verifica o shell do container). Veja [DependsOn](#dependson).
