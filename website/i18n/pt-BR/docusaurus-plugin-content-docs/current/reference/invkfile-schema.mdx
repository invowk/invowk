...
Invkfiles são validados com structs fechadas, então campos desconhecidos geram erro de validação.
...
Allowlist de variáveis do host quando `env_inherit_mode` é `"allow"`.
...
Denylist de variáveis do host (aplica a qualquer modo).
