...
Configuração padrão de backend LLM. `DefaultConfig()` deriva esse valor do CUE e produz um objeto `llm: {}` vazio, sem backend, até o usuário configurar um provedor ou endpoint de API.
