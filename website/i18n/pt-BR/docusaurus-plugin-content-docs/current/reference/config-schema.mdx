...
Configuração padrão de backend LLM para comandos que usam LLM. `invowk agent cmd create|change` e `invowk agent mod create|change` usam essa configuração quando nenhuma flag LLM é passada. `invowk audit` ainda exige opt-in explícito com `--llm` ou `--llm-provider` antes de enviar conteúdo de script para uma LLM.
