...
O Invowk valida nomes de interpretadores contra uma lista de permissão por segurança ([SC-08](https://github.com/invowk/invowk)). Os seguintes interpretadores são aceitos:
...
Valores não-shell em `script.interpreter` **não são suportados** com o runtime virtual:

<Snippet id="advanced/interpreter-virtual-error" />

O runtime virtual usa o interpretador mvdan/sh embutido e não pode executar Python, Ruby ou outros interpretadores. Use runtime native ou container em vez disso. Configurações de runtime não aceitam `interpreter`; uma forma como `runtimes: [{name: "native", interpreter: "python3"}]` é inválida.
...
- [Diretório de Trabalho](./workdir) - Controlar local de execução
