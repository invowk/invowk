...
Packs são pastas autocontidas que agrupam metadados `invkpack.cue` com comandos opcionais em `invkfile.cue` e quaisquer arquivos de script que eles referenciam. Eles são perfeitos para compartilhar comandos, criar toolkits reutilizáveis e distribuir automação entre equipes.
...
- **Autocontido**: Scripts ficam agrupados no pack
...
- **`invkpack.cue`**: Metadados do pack (nome, versão, dependências)

### Conteúdo Opcional

- **`invkfile.cue`**: Definições de comandos (omita para packs apenas de biblioteca)
...
- [Dependências de Packs](./dependencies/overview) - Usar outros packs no seu pack
