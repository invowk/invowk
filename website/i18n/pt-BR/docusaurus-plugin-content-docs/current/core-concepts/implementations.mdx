...
O **primeiro runtime** é o padrão. Usuários podem sobrescrever com `--invk-runtime`:
...
2. **Runtime solicitado** - Se `--invk-runtime` especificado, usa esse; caso contrário usa o padrão
...
| Linux | `invowk cmd myproject build --invk-runtime virtual` | Primeira impl, runtime virtual |
| Windows | `invowk cmd myproject build` | Segunda impl, runtime native |
| Windows | `invowk cmd myproject build --invk-runtime virtual` | Erro: nenhuma impl correspondente |
