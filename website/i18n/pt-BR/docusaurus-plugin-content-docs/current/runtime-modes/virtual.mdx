...
O runtime virtual **não é um sandbox de segurança**. Ele é um interpretador de shell portável com utilitários integrados. Qualquer comando não fornecido pelos built-ins (ex.: `git`, `curl`, `python`, `node`) é resolvido a partir do `PATH` do sistema host e executado como um processo nativo com acesso completo ao host. Por padrão, as variáveis de ambiente do host também são herdadas (`env_inherit_mode: "all"`).
...
O runtime virtual **não pode** usar valores não-shell em `script.interpreter`:
...
- **Sem dependência de shell**: Funciona mesmo se o shell do sistema não estiver disponível (o interpretador é integrado, mas comandos externos ainda requerem binários do host)
