...
| `INVOWK_SSH_TOKEN` | Token de autenticação com escopo da execução |

### Segurança

- Cada execução de comando recebe um token único
- Tokens são revogados quando o comando termina, o que também fecha qualquer conexão SSH aberta com eles
...
O auto-provisionamento se aplica a execuções de container baseadas em imagem, incluindo execuções não interativas e interativas. Quando um comando aponta para um container persistente existente, o Invowk pula a preparação da imagem e usa esse container diretamente. Para execuções baseadas em imagem, a imagem derivada é cacheada como `invowk-provisioned:<hash>` e reutilizada; se o provisionamento falhar, o Invowk avisa e usa a imagem base apenas no modo padrão não estrito. Com `container.auto_provision.strict: true`, a falha de provisionamento interrompe o comando.
...
- [Runtime Virtual-Sh](./virtual) - Para scripts multiplataforma
