...
O comando `invowk audit` constrói um snapshot imutável de todos os artefatos descobertos (invowkfiles, módulos, scripts, lock files), e então executa **7 checkers de segurança integrados** concorrentemente. Após todos os checkers completarem, um **correlacionador** cruza os achados para detectar ameaças compostas.
...
invowk audit [path]
  ...
  │
  ├── Checkers Concorrentes (7 integrados + LLM opcional)
  │   ├── Script Checker ──► achados de execução, travessia, ofuscação
  │   ├── Lua Checker ──► achados de execução, env e caminhos em virtual-lua
  │   ├── Network Checker ──► achados de execução e exfiltração
...
Analisa scripts virtual-lua e configuração de runtime em busca de acesso ao host e uso indevido da ponte.

**Detecta:**
- Referências a APIs Lua desabilitadas (`os.execute`, `io.popen`, `package.loadlib`, `debug` e helpers de carregamento direto de arquivos)
- Leituras de ambiente sensível via `os.getenv()` ou pela ponte de ambiente Lua do Invowk
- Entradas wildcard ou capazes de rede em `allowed_binaries` que permitem ao Lua invocar comandos amplos do host
- Acesso completo ao sistema de arquivos virtual ou mapeamentos de caminhos amplos expostos às APIs de arquivo Lua
...
**Detecta:**
- Padrões de reverse shell (bash, Python, netcat)
...
**Detecta:**
- `env_inherit_mode: "all"` arriscado e defaults não definidos de `env_inherit_mode` nos runtimes native/virtual-sh (ambos expõem todas as variáveis de ambiente do host)
...
**Detecta:**
- **Typosquatting**: IDs de módulos similares a outro ID de módulo no scan atual (distância de Levenshtein)
...
- **Módulos vendorizados ausentes de `requires`**: Dependências presentes em `invowk_modules/` sem uma declaração correspondente
- **Falhas de parsing do invowkfile do módulo**: Arquivos de comando de módulo malformados que impedem auditoria do conteúdo de scripts
- **Confiança em módulos globais**: Módulos de `~/.invowk/cmds/` que contornam a revisão local

## Detecção de Ameaças Compostas

O correlacionador identifica combinações específicas na mesma superfície de
ataque. Quatro regras combinam checkers diferentes; a travessia de interpretador
combina duas categorias do próprio checker de Script:

| Ameaça Composta | Achados Obrigatórios | Severidade |
|-----------------|----------------------|------------|
| **Exfiltração de credenciais** | Checker de Env + checker de Network | Crítica |
| **Escape path + symlink** | Checker de Script + checker de Symlink | Crítica |
| **Exfiltração ofuscada** | Script/`obfuscation` + Network/`exfiltration` | Crítica |
| **Fraqueza na cadeia de confiança** | Um código de confiança do grafo de dependências de Module Metadata + um código de integridade ausente, obsoleto ou divergente de Lock File | Alta |
| **Travessia de interpretador** | Script/`execution` + Script/`path-traversal` | Crítica |

A regra de cadeia de confiança aceita apenas os códigos de metadados para fan-out
amplo, dependências transitivas não declaradas ou módulos vendorizados ausentes de
`requires`. No lado do lock file, aceita os códigos definidos para lock ausente,
v1 legado, entrada ausente, módulo vendorizado não verificável ou divergência de
content hash; achados não relacionados desses checkers não ativam a regra.
...
| Código | Significado |
|--------|-------------|
| `0` | Nenhum achado confirmado no ou acima do limiar de severidade; relatórios apenas com achados suprimidos saem com sucesso |
| `1` | Achados confirmados detectados; erros genéricos de validação de argumentos da CLI também usam o status 1 padrão do Cobra |
| `2` | Erro de scan de auditoria ou configuração |
...
O checker LLM com backend de API funciona com servidores compatíveis com OpenAI
que oferecem chat completions e listagem de modelos. O Invowk sempre verifica o
modelo configurado de um backend HTTP/API pelo endpoint `/models` antes de
escanear; uma falha de listagem ou verificação encerra a auditoria com status 2.
Provedores CLI de Claude, Codex e Gemini delegam o tratamento do modelo à
ferramenta CLI. A configuração de API padrão usa o [Ollama](https://ollama.com),
o servidor LLM local mais popular:
...
Para todo backend HTTP/API selecionado, o Invowk lista os modelos e verifica o
modelo configurado antes de escanear. Se o endpoint falhar, a auditoria termina
com status 2. Se o modelo não for encontrado, o Invowk mostra:
...
O checker LLM:
1. **Verifica** pelo endpoint obrigatório de listagem se um backend HTTP/API possui o modelo configurado (com sugestões se não); provedores CLI delegam o tratamento do modelo à sua ferramenta
2. **Resolve** conteúdo de scripts inline e baseados em arquivo, depois filtra scripts vazios
...
6. **Rejeita** campos de achado malformados, como valores desconhecidos de severidade/categoria, em vez de tratar silenciosamente uma resposta ruim do LLM como limpa
7. **Integra** achados válidos no mesmo pipeline dos checkers integrados

Quando a análise LLM é solicitada explicitamente, falhas do checker LLM são erros fatais de varredura. Por exemplo, campos de achado LLM malformados, como valores desconhecidos de severidade ou categoria, fazem a auditoria falhar com código de saída 2 em vez de cair para resultados determinísticos parciais.

Achados do LLM participam da detecção de ameaças compostas pelo mesmo pipeline de correlação dos achados integrados. Eles podem contribuir para resultados **Críticos** quando uma regra nomeada também corresponde (por exemplo, acesso a variáveis de ambiente sensíveis junto com acesso de rede) ou quando condições genéricas de escalonamento são atendidas, como um achado de alta severidade junto com qualquer outro achado ou três categorias distintas na mesma superfície.

:::caution O conteúdo dos scripts é enviado ao backend LLM selecionado
Tanto `--llm` quanto `--llm-provider` habilitam a análise do conteúdo dos scripts dos seus invowkfiles e módulos. Backends de API o recebem por HTTP; provedores CLI de Claude, Codex e Gemini o recebem pelo subprocesso da ferramenta selecionada e podem encaminhá-lo conforme o comportamento dessa ferramenta. Servidores de API locais mantêm o payload HTTP na sua máquina. Para APIs em nuvem e provedores CLI, revise as políticas de tratamento de dados do provedor e da ferramenta.
:::
...
| `summary.suppressed` | Contagem de achados suprimidos e ameaças compostas suprimidas no limiar de severidade ou acima dele |
...
Cada achado inclui `code`, `severity`, `category`, `checker_name`, `title`, `description` e `recommendation`, além de `surface_id`, `surface_kind`, `file_path`/`line` e campos de proveniência de escalação opcionais. Valores válidos de `surface_kind` são `root_invowkfile`, `local_module`, `vendored_module` e `global_module`.
