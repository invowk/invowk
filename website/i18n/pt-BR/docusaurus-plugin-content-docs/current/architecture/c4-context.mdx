...
|---------|-----------|-----------|
| **Docker Engine** | Runtime de container para execução isolada de comandos. Fornece ambientes reproduzíveis. | Docker CLI |
...
| **Provedores LLM Opcionais** | Análise com LLM para `invowk audit` e autoria create/change com `invowk agent cmd` e `invowk agent mod` via Claude, Codex/OpenAI, Gemini ou Ollama local. | APIs HTTP compatíveis com OpenAI / CLIs locais |
