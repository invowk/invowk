...
| 3 | Includes configurados | `/opt/invowk-modules/tools.invkmod` |
| 4 (menor) | Diretório de comandos do usuário | `~/.invowk/cmds/mytools.invkmod/` |
...
- O diretório de comandos do usuário (`~/.invowk/cmds/`) descobre apenas diretórios `*.invkmod` como filhos imediatos (não recursivo). Invkfiles avulsos não são descobertos a partir desse local.
