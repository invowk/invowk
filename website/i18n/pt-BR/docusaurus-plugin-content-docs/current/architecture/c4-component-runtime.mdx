...
| **ContainerRuntime** | Go | Executa comandos dentro de containers Docker/Podman. Depende de `container.Engine`, da porta `provision.Provisioner` para provisionamento de imagens de container, da porta `runtime.HostCallbackServer` para callbacks opcionais do host e `config.Config`. Somente containers Linux. Implementa Runtime, CapturingRuntime e InteractiveRuntime. |
...
| `provision.Provisioner` | ContainerRuntime | Porta para provisionamento de imagens de container; `provision.LayerProvisioner` é o adaptador de produção que cria camadas de imagem efêmeras com o binário invowk e os módulos |
