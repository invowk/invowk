...
| **ContainerRuntime** | Go | Executa comandos dentro de containers Docker/Podman. Depende de `container.Engine`, `provision.LayerProvisioner`, a porta `runtime.HostCallbackServer` para callbacks opcionais do host e `config.Config`. Somente containers Linux. Implementa Runtime, CapturingRuntime, InteractiveRuntime e HostServiceAddressProvider. |
