...
| **Agent Authoring** | Go | Autoria de comandos e módulos locais assistida por LLM (`internal/agentcmd/`, `internal/llm/`). Renderiza prompts por operação, resolve backends LLM configurados, valida CUE gerado, aplica patches em `invowkfile.cue` e grava bundles limitados a `invowkmod.cue`/`invowkfile.cue`. |
...
1. **Discovery Engine** lê invowkfiles do diretório atual, módulos locais, includes configurados, entradas de módulos provisionadas, entradas globais provisionadas, módulos de comandos do usuário e módulos vendorizados de um nível
