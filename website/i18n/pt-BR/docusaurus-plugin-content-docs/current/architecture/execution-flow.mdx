...
- Search paths para invkfiles/módulos
...
| 6-7 | CUE Parser | Fazer parsing de todos os arquivos `invkfile.cue` |
| 8-9 | CUE Parser | Fazer parsing de todos os diretórios `*.invkmod` |
| 10 | Discovery | Construir árvore unificada de comandos |

**Ordem de precedência (maior para menor):**
1. `invkfile.cue` do diretório atual
2. `*.invkmod` do diretório atual
3. Diretório do usuário `~/.invowk/cmds/`
4. Search paths configurados
...
- Verificar `platforms: ["linux"]`, `["darwin"]`, `["windows"]`
...
- Invkfiles grandes → parsing mais lento
