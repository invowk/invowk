...
Este diagrama amplia o container **Container Engine Abstraction** do [Diagrama de Container C2](./c4-container) para mostrar os componentes internos do pacote `internal/container` e as portas adjacentes de runtime, planejamento e provisionamento que os consomem. Ele revela como o Invowk fornece uma interface unificada sobre os CLIs do Docker e Podman, aplica labels de volume SELinux nos dois engines, lida com preocupações rootless específicas do Podman e se adapta de forma transparente a ambientes sandboxed (Flatpak, Snap).
...
|------------|------------|------------------|
| **Engine** | Interface Go | Contrato de 12 métodos para ciclo de vida e execução: `Name`, `Available`, `Version`, `Build`, `Run`, `InspectContainer`, `Create`, `Start`, `Exec`, `Remove`, `ImageExists`, `RemoveImage`. Todas as funções factory retornam este tipo. |
| **CommandPreparer** | Interface Go | Contrato do adaptador interativo: `BinaryPath`, `BuildRunArgs`, `PrepareRunCommand`. Mantido separado de `Engine`. |
| **LifecycleCoordinator** | Interface Go | Coordena operações de ciclo de vida que precisam da mesma política de serialização de engine usada por runs de container. |

## Implementações

| Componente | Tecnologia | Responsabilidade |
|------------|------------|------------------|
| **BaseCLIEngine** | Struct Go | Base compartilhada para engines baseados em CLI. Fornece construtores de argumentos (`BuildArgs`, `RunArgs`, `ExecArgs`, `CreateArgs`, `StartArgs`), operações de inspect/create/start/exec, preparação de comandos e coordenação de ciclo de vida. Configurado via functional options. |
| **DockerEngine** | Struct Go | Embarca `*BaseCLIEngine`. Implementa `Engine`. Localiza o binário `docker` via `exec.LookPath`. Injeta labels de volume SELinux (`:z`) quando o daemon reporta SELinux em `docker info`. |
...
| **VolumeFormatFunc** | Transforma valores `VolumeMountSpec` antes de passá-los para `-v`. Docker e Podman usam para adicionar labels SELinux (`:z`). |
| **RunArgsTransformer** | Pós-processa argumentos de run. Podman usa para injetar `--userns=keep-id` para compatibilidade rootless. |
| **SELinuxCheckFunc** | Determina se labeling SELinux deve ser aplicado. Injetado nos formatters de volume dos dois engines: o Podman verifica o host local, o Docker consulta seu daemon. |
...
| **CreateOptions** | Entrada para `Engine.Create()`: imagem, comando idle, diretório de trabalho, env, labels, volumes, portas, nome, extra hosts. |
| **CreateResult** | Saída de `Engine.Create()`: ID do container. |
| **RunResult** | Saída de `Engine.Run()`: ID do container, exit code, erro. |
| **ContainerInfo** | Saída de `Engine.InspectContainer()`: ID do container, nome, estado running, status, labels. |
...
| Consumidor ou Política | Contrato | Relacionamento |
|------------------------|----------|----------------|
| **runtime.ContainerRuntime** | Porta privada `runtime.containerEngine` | Armazena o contrato restrito de engine necessário para build de imagem, runs avulsos e operações persistentes de inspect/create/start/exec. As implementações de engine Docker, Podman e com reconhecimento de sandbox satisfazem essa porta. |
| **provision.Provisioner** | Porta de provisionamento de imagem armazenada pelo `ContainerRuntime` | `LayerProvisioner` é a implementação de produção criada pelos construtores de runtime. |
| **provision.LayerProvisioner** | Porta privada `provision.imageBuilder` | Usa as operações `Build` e `ImageExists` do engine sem depender do contrato completo de `container.Engine`. |
| **runtime.persistentContainerPlan** | `containerplan.ResolvePersistentTarget` | Delega a decisão entre alvo efêmero e persistente para `internal/containerplan`; depois, `ContainerRuntime` aplica o `PersistentPlan` retornado por meio das operações de ciclo de vida do engine. |
...
- **Comportamento específico do engine**: os dois construtores adicionam `WithVolumeFormatter` (SELinux); o do Podman também adiciona `WithRunArgsTransformer` (rootless userns). Ambos vêm antes das options fornecidas pelo usuário.
