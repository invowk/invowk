...
- **Nomes reservados no Windows**: Arquivos chamados `CON`, `PRN`, `AUX`, `NUL`, `COM1`-`COM9` ou `LPT1`-`LPT9` criam um problema de validação de compatibilidade. O módulo é inválido e `invowk validate` termina com status 1 porque esses nomes não podem ser extraídos de modo confiável no Windows.
