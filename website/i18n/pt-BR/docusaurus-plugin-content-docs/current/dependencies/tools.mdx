...
## Validação no Host e no Container

Verificações de ferramentas no nível raiz, de comando e de implementação são sempre validadas no host. Para validar ferramentas dentro de uma imagem de container, coloque `depends_on` dentro do bloco do runtime container selecionado.
...
Ferramentas no nível de implementação ainda são verificadas no host antes de um runtime virtual-sh ou virtual-lua iniciar:

<Snippet id="dependencies/tools-runtime-virtual" />

### Runtime Container

Verificações de ferramentas no nível do runtime container são verificadas **dentro do container**:
