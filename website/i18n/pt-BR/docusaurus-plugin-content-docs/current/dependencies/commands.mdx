...
Se uma dependência de comando for declarada em `requires`, mas ainda falhar na verificação de acesso, execute `invowk module sync` para que o lock file registre o ID de módulo estável e o namespace de fonte de comando da dependência. Se ainda falhar, a cópia descoberta difere do conteúdo travado: alinhe a versão da dependência com a que seu módulo travou.
