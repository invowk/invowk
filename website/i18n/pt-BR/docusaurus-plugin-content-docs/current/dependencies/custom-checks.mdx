...
| `script.content` | Sim | Conteúdo inline do script a executar para a verificação |
| `script.file` | Somente módulo | Arquivo de script contido no invowkmod de origem |
| `script.interpreter` | Não | Interpretador para o conteúdo resolvido do script; por padrão usa detecção por shebang e depois o shell mvdan/sh embutido para verificações no host |
...
Custom checks no host sem shebang ou `script.interpreter` rodam pelo shell mvdan/sh embutido do Invowk. Um shebang não-shell ou `script.interpreter` como `python3` roda por esse interpretador no host. Custom checks em container usam o mesmo campo `script.interpreter` dentro do container selecionado. Se um `script.interpreter` concreto sobrescrever um shebang diferente, o Invowk reporta um aviso informativo e mantém o interpretador explícito como autoridade.
