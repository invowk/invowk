...
Custom checks no host sem shebang ou `script.interpreter` rodam pelo shell mvdan/sh embutido do Invowk. Um shebang não-shell ou `script.interpreter` como `python3` roda por esse interpretador no host. Custom checks em container usam o mesmo campo `script.interpreter` dentro do container selecionado.
