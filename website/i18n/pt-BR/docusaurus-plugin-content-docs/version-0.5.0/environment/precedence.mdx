...
Aplicada da menor para a maior prioridade:

| Prioridade | Fonte | Exemplo |
|------------|-------|---------|
| 1 | Ambiente do sistema | Ambiente do host (se `env_inherit_mode` permitir) |
| 2 | Arquivos raiz | `root.env.files` |
| 3 | Arquivos de comando | `command.env.files` |
| 4 | Arquivos de implementação | `implementations[].env.files` |
| 5 | Vars raiz | `root.env.vars` |
| 6 | Vars de comando | `command.env.vars` |
| 7 | Vars de implementação | `implementations[].env.vars` |
| 8 | Vars do Invowk | `INVOWK_*`, `ARG*` |
| 9 | Arquivos env da CLI | `--ivk-env-file .env.local` |
