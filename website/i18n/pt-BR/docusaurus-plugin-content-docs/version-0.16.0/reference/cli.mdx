...
| `--ivk-container-name` | | Sobrescrever o nome do container persistente de destino (somente runtime container) |
...
| `--ivk-env-inherit-allow` |  | Allowlist de variáveis do host; exige `--ivk-env-inherit-mode=allow` (repetível) |
...
- `default` - Um comando `hello` demonstrando o padrão de divisão por plataforma nas opções de runtime integradas
- `minimal` - Estrutura mínima de invowkfile
- `full` - Template que demonstra subcomandos, flags, configuração de ambiente e dependências de comandos
...
| Flag | Descrição |
|------|-----------|
| `--alias` | Alias para o ID de origem de comando do módulo |
...
| Caminho | Validação |
|---------|-----------|
| Diretório `*.invowkmod` | Validação de módulo (estrutura, mais parsing de invowkfile e árvore de comandos quando `invowkfile.cue` está presente) |
...
:::tip
`invowk validate` sempre executa validação profunda da estrutura e dos metadados do módulo.
Quando o módulo contém `invowkfile.cue`, também faz o parsing do arquivo e valida sua árvore
de comandos; módulos apenas de biblioteca não executam essas verificações específicas.
:::
...
Imprimir system prompts por operação que agentes LLM devem usar ao criar comandos personalizados do Invowk.

<Snippet id="reference/cli/agent-prompt-syntax" />

O argumento opcional `operation` aceita `create`, `change` ou `remove`. Omita-o para imprimir os prompts das três operações.
...
Gerar um comando personalizado com uma LLM, validá-lo e aplicar o patch em `invowkfile.cue`. O `name` gerado deve corresponder a `<name>`. Veja [Autoria de Agente com LLM](../advanced/llm-assisted-authoring) para o fluxo completo.
...
### invowk agent cmd change

Gerar uma substituição para um comando personalizado existente com uma LLM. O `name` gerado deve continuar correspondendo a `<name>`, e apenas esse comando é substituído.

<Snippet id="reference/cli/agent-change-syntax" />

**Flags:** iguais a `agent cmd create`.

### invowk agent cmd remove

Remover um comando personalizado de um invowkfile sem resolver um provedor LLM.

<Snippet id="reference/cli/agent-remove-syntax" />
...
Imprimir system prompts por operação que agentes LLM devem usar ao criar módulos locais do Invowk.

<Snippet id="reference/cli/agent-mod-prompt-syntax" />
...
### invowk agent mod create

Gerar `invowkmod.cue` e `invowkfile.cue` para `<module-id>.invowkmod` com uma LLM. O ID de módulo gerado deve corresponder a `<module-id>`.

<Snippet id="reference/cli/agent-mod-create-syntax" />

**Flags:**

| Flag | Padrão | Descrição |
|------|--------|-----------|
| `--from-file` | | Ler a solicitação do módulo a partir de um arquivo |
| `--dry-run` | `false` | Imprimir os patches sem escrever |
| `--print` | `false` | Imprimir o pacote de arquivos gerado sem escrever |
| `--verify` | `false` | Depois de escrever, validar o módulo gerado |
| `--scripts` | `false` | Criar um diretório `scripts/` vazio com `.gitkeep` |
| `--llm-provider` | | Auto-detectar ou forçar provedor: `auto`, `claude`, `codex`, `gemini`, `ollama` |
| `--llm` | `false` | Habilitar LLM usando configuração ou API |
| `--llm-url` | `http://localhost:11434/v1` | URL base da API compatível com OpenAI |
| `--llm-model` | Fallback de API/Ollama: `qwen2.5-coder:7b`; padrão da ferramenta para provedor CLI quando omitido | Nome do modelo; obrigatório para provedores de API cloud e opcional para provedores CLI |
...
### invowk agent mod change

Gerar substituições para `invowkmod.cue` e `invowkfile.cue` em um módulo local existente. Nenhum outro arquivo do módulo é gerado ou editado.

<Snippet id="reference/cli/agent-mod-change-syntax" />

**Flags:** iguais a `agent mod create`, exceto que `--scripts` não é suportado.

### invowk agent mod remove

Remover um diretório de módulo local validado sem editar dependências ou lock files. A exclusão exige `--force`; use `--dry-run` para visualizar o plano.

<Snippet id="reference/cli/agent-mod-remove-syntax" />

**Flags:**

| Flag | Padrão | Descrição |
|------|--------|-----------|
| `--dry-run` | `false` | Imprimir o plano de exclusão sem remover o módulo |
| `--force` | `false` | Excluir o diretório de módulo validado |
