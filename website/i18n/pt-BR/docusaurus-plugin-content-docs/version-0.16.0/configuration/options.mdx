...
Habilita utilitários [u-root](https://github.com/u-root/u-root) no `virtual-sh` e helpers de comando no `virtual-lua`. Quando habilitado, 28 comandos adicionais compatíveis com POSIX ficam disponíveis para o `virtual-sh`:
...
Configura um backend LLM padrão para comandos que usam LLM. `invowk agent cmd create|change` e `invowk agent mod create|change` usam essa configuração automaticamente quando nenhuma flag LLM é passada. `invowk audit` ainda exige opt-in explícito com `--llm` ou `--llm-provider` antes de enviar qualquer conteúdo de script para um LLM.
...
**Tipo:** string de duração Go (`#LLMTimeoutDurationString`, máximo de 64 runes)
...
Se `api` estiver presente, defina pelo menos um desses campos.
