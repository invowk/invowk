...
|------------|-----------|
| `local-area-network` | Detecta uma interface ativa que não seja loopback e tenha um endereço IP roteável |
...
Verifique a disponibilidade de rede local:

<Snippet id="dependencies/capabilities-lan" />

Isso verifica:
- Existe uma interface de rede ativa que não seja loopback
- A interface possui um endereço IP roteável

Essa verificação não envia tráfego nem prova que um serviço específico da LAN esteja acessível.
...
Para runtime container, verificações de capacidade no nível raiz, de comando e de implementação rodam no **host**. Verificações de capacidade declaradas dentro da configuração do runtime container selecionado rodam **dentro do container**:

<Snippet id="dependencies/capabilities-container-context" />

Use verificações no host para alcance de deploy e outros requisitos da estação de trabalho. Use verificações no nível do runtime container quando o comando precisa que a própria imagem do container prove que um serviço é alcançável ou que uma capacidade está disponível dentro do ambiente de execução.
