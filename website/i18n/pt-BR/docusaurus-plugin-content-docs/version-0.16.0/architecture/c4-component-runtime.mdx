...
:::note
O pacote runtime (`internal/runtime/`) é responsável por executar comandos definidos pelo usuário. Ele fornece quatro backends de execução intercambiáveis por trás de uma hierarquia de interfaces comum, um registry para despacho de runtime, e um contexto de execução estruturado que desacopla I/O, ambiente e preocupações de TUI.
:::
...
| **HostServiceAddressProvider** | `HostServiceAddress()` | Capacidade opcional para runtimes cujos processos filhos precisam de um endereço diferente de localhost para alcançar serviços do host, como o servidor TUI. Implementada por ContainerRuntime. |
...
| **ShRuntime** | Go/mvdan-sh | Interpretador de shell POSIX embarcado com utilitários built-in u-root opcionais. Sem dependência de shell do host. Cria um subprocesso de si mesmo para modo interativo baseado em PTY. Implementa Runtime, CapturingRuntime e InteractiveRuntime. |
| **LuaRuntime** | Go/golua | Runtime Lua embarcado com o harness de segurança virtual compartilhado. Implementa Runtime, CapturingRuntime e InteractiveRuntime. |
| **ContainerRuntime** | Go | Executa comandos dentro de containers Docker/Podman. Depende da porta privada `containerEngine`, da porta `provision.Provisioner`, da porta `runtime.HostCallbackServer` para callbacks opcionais do host e de `config.Config`. Os construtores de runtime fornecem uma implementação de `container.Engine` e constroem `provision.LayerProvisioner` como provisionador de produção. Somente containers Linux. Implementa Runtime, CapturingRuntime, InteractiveRuntime e HostServiceAddressProvider. |
...
| **RuntimeType** | Tipo string identificando variantes de runtime: `"native"`, `"virtual-sh"`, `"virtual-lua"`, `"container"`. |
...
| **Result** | Resultado de execução: `ExitCode`, `Error`, `Output` (stdout capturado), `ErrOutput` (stderr capturado) e `Diagnostics []InitDiagnostic` para avisos não fatais de inicialização/provisionamento do runtime. O serviço de comandos converte esses diagnósticos no resultado de execução para renderização. |
...
|-------------|-----------|-----------|
| Implementações de `container.Engine` | Porta privada `containerEngine` do ContainerRuntime | Os adaptadores de engine Docker/Podman satisfazem as operações restritas necessárias ao runtime |
| `provision.Provisioner` | ContainerRuntime | Porta de provisionamento de imagem; os construtores de runtime usam `provision.LayerProvisioner` como implementação de produção |
| `runtime.HostCallbackServer` | ContainerRuntime | Porta que fornece credenciais de callback do host baseadas em token para execuções em container; `internal/app/commandadapters` adapta o `sshserver.Server` concreto para essa porta |
| `config.Config` | ContainerRuntime | Configuração da aplicação (preferência de engine de container, etc.) |
| `mvdan.cc/sh/v3` | ShRuntime | Interpretador de shell POSIX embarcado |
| `github.com/invowk/golua` | LuaRuntime | Interpretador Lua embarcado (fork mantido pela invowk de arnodel/golua, com patch para a remoção de linknames no Go 1.27) |
| `internal/uroot` | virtual-sh e virtual-lua | Utilitários built-in para a família de runtimes virtuais (cp, mv, cat, etc.) |
...
Native, Sh, Lua e Container implementam a interface interativa.
...
O padrão Registry mantém os construtores de runtime atrás do adapter de comando. Um registry preenchido e uma sessão de runtime são criados para cada execução de comando, então a inicialização e a limpeza de containers continuam limitadas à execução enquanto o pipeline simplesmente pede um runtime por tipo. Adicionar um novo runtime significa registrá-lo -- sem mudanças no pipeline de execução.
