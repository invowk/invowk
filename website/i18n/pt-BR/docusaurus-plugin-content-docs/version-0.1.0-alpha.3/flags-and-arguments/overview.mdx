...
- `invk-env-file` / `-e` - Carregar ambiente de arquivo
- `invk-env-var` / `-E` - Definir variável de ambiente
- `invk-workdir` / `-w` - Sobrescrever diretório de trabalho
- `invk-runtime` / `-r` - Sobrescrever runtime

Além disso, os prefixos `invk-`, `invowk-` e `i-` são reservados para flags do sistema. Não use esses prefixos ou nomes para suas flags.
