...
| `--invk-config` | `-c` | Caminho do arquivo de configuração (padrão: `$HOME/.config/invowk/config.cue`) |
| `--invk-interactive` | `-i` | Executar comandos em buffer de tela alternativo (modo interativo) |
| `--invk-verbose` | `-v` | Habilitar saída verbose |
...
| Flag | Curta | Descrição |
|------|-------|-----------|
| `--invk-runtime` | `-r` | Sobrescrever o runtime (deve ser permitido pelo comando) |

Flags embutidas por comando:

| Flag | Curta | Descrição |
|------|-------|-----------|
| `--invk-env-file` | `-e` | Carregar variáveis de ambiente de arquivo (repetível) |
| `--invk-env-var` | `-E` | Definir variável de ambiente (KEY=VALUE, repetível) |
| `--invk-env-inherit-mode` |  | Herdar variáveis do host: `none`, `allow`, `all` |
| `--invk-env-inherit-allow` |  | Allowlist de variáveis do host (repetível) |
| `--invk-env-inherit-deny` |  | Denylist de variáveis do host (repetível) |
| `--invk-workdir` | `-w` | Sobrescrever diretório de trabalho do comando |

:::note
Os prefixos `invk-`, `invowk-` e `i-` são reservados para flags do sistema e não podem ser usados para flags definidas pelo usuário.
:::
...
3. Entradas configuradas em `includes` (invkfiles e módulos)
...
- Nenhum invkfile existe nos `includes` configurados

**Soluções:**
- Execute `invowk init` para criar um invkfile no diretório atual
- Navegue para um diretório que contenha um invkfile
- Adicione caminhos de invkfile ou módulo em `includes` na sua configuração
...
- Rode com modo verbose: `invowk --invk-verbose cmd`
