...
| Componente | Tecnologia | Responsabilidade |
|------------|------------|------------------|
| **Discovery Engine** | Go | Encontra diretórios `invkfile.cue` e `*.invkmod` com ordenação por precedência. Constrói árvore unificada de comandos. |
...
| **Module Resolver** | Go | Resolve dependências baseadas em Git. Gerencia cache em `~/.cache/invowk/modules/`. Lida com lock files para reprodutibilidade. |
...
| **Invkfiles** | CUE | `./invkfile.cue`, search paths | Definições de comandos com implementações |
| **Modules** | Diretórios | `*.invkmod/` | Comandos empacotados com metadados `invkmod.cue` |
| **Module Cache** | Sistema de Arquivos | `~/.cache/invowk/modules/` | Módulos remotos buscados via Git em cache |
...
4. **CUE Parser** faz parsing de `invkfile.cue` e arquivos de módulos
