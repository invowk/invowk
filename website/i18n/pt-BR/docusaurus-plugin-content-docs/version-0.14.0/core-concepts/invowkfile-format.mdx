...
Scripts podem ser inline. Dentro de módulos invowk, eles também podem referenciar arquivos contidos no mesmo módulo:
...
`script.file` é permitido apenas em módulos e precisa ficar contido no próprio módulo. Ele não é detectado por extensão; arquivos de script sem extensão são válidos. Em invowkfiles de raiz/projeto, chame scripts locais por `script.content`.
