...
| `invowk module vendor` | Faz vendor das dependências em `invowk_modules/` (parcial) |

## invowk module add

Resolve uma dependência e imprime a entrada de `requires` para adicionar em `invowkmod.cue`.
...
:::note
`invowk module add` não edita `invowkmod.cue` automaticamente. Copie a entrada de `requires` impressa para os metadados do módulo e execute `invowk module sync`.
:::
...
:::note
Esse comando remove a dependência apenas do lock file. Você precisa remover manualmente a entrada de `requires` de `invowkmod.cue`. Arquivos de cache não são deletados.
:::
