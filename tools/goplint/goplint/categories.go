...
func diagnosticCategoryRegistry() []CategorySpec {
	return []CategorySpec{
		...
		{Name: CategoryPathmatrixDivergent, BaselinePolicy: BaselineSuppressible, BaselineLabel: "pathmatrix.PassRelative on a platform-divergent vector without OnWindows override"},
		{Name: CategoryTestHomeEnvPlatform, BaselinePolicy: BaselineAlwaysVisible},
		{Name: CategoryMissingCommandWaitDelay, BaselinePolicy: BaselineSuppressible, BaselineLabel: "exec.CommandContext used without setting Cmd.WaitDelay before process execution"},
		{Name: CategoryCueFedPathNativeClean, BaselinePolicy: BaselineSuppressible, BaselineLabel: "CUE-fed or repo-relative path validated through host-native filepath cleanup before slash normalization"},
