...
Committed baselines live under `tools/mutation/baselines/`. The first accepted-survivor pass was generated from completed advisory full scans, then immediately tightened with focused tests for high-value survivors. The current committed baselines accept 372 root-module survivors and 810 `tools/goplint` survivors. Treat baseline updates as explicit review events: kill high-value survivors with focused tests first, then run `make mutation-baseline-update MUTATION_MODULE=<module>` or regenerate from a reviewed full report to remove killed historical survivors and accept the remaining current set.
