...
C4Container
    ...
    Container_Boundary(entry_core, "Invowk CLI - Entry & Core") {
        Container(cmd, "CLI Commands", "Go/Cobra", "Root, cmd, init, config, module, tui, completion")
        Container(discovery, "Discovery Engine", "Go", "Locates invkfiles and modules")
        Container(config, "Configuration Manager", "Go/Viper+CUE", "Loads and validates config")
        Container(cue_parser, "CUE Parser", "Go/cuelang", "3-step parsing: schema, unify, decode")
        Container(module_resolver, "Module Resolver", "Go", "Git dependency resolution with cache")
    }

    Container_Boundary(runtimes, "Invowk CLI - Runtimes") {
        Container(runtime_native, "Native Runtime", "Go", "Host shell execution")
        Container(runtime_virtual, "Virtual Runtime", "Go/mvdan-sh", "Embedded shell with u-root")
        Container(runtime_container, "Container Runtime", "Go", "Docker/Podman execution")
    }

    Container_Boundary(container_infra, "Invowk CLI - Container Infrastructure") {
        Container(container_engine, "Container Engine", "Go", "Docker/Podman abstraction")
        Container(provision, "Image Provisioner", "Go", "Ephemeral layers with invowk")
        Container(ssh_server, "SSH Server", "Go/Wish", "Container-to-host callbacks")
        Container(tui_server, "TUI Server", "Go/Bubble Tea", "HTTP for TUI requests")
    }

    ContainerDb(config_file, "Config File", "CUE", "~/.config/invowk/config.cue")
    ContainerDb(invkfiles, "Invkfiles", "CUE", "invkfile.cue definitions")
    ContainerDb(modules, "Modules", "Directories", "*.invkmod directories")
    ContainerDb(cache, "Module Cache", "Filesystem", "~/.cache/invowk/modules/")

    %% User to CLI
    Rel(user, cmd, "Runs commands", "CLI")

    %% Entry & Core relationships
    Rel(cmd, discovery, "Discovers")
    Rel(cmd, config, "Loads config")
    Rel(discovery, cue_parser, "Parses")
    Rel(discovery, module_resolver, "Resolves deps")
    Rel(config, cue_parser, "Parses")

    %% Data store access
    Rel(discovery, invkfiles, "Reads")
    Rel(discovery, modules, "Reads")
    Rel(config, config_file, "Reads")
    Rel(module_resolver, git, "Fetches", "Git")
    Rel(module_resolver, cache, "Caches")

    %% Command to Runtime fan-out
    Rel(cmd, runtime_native, "Executes")
    Rel(cmd, runtime_virtual, "Executes")
    Rel(cmd, runtime_container, "Executes")

    %% Runtime to External/Infrastructure
    Rel(runtime_native, host_shell, "Spawns", "exec")
    Rel(runtime_virtual, tui_server, "Requests TUI", "HTTP")
    Rel(runtime_container, container_engine, "Runs")
    Rel(runtime_container, provision, "Provisions")
    Rel(runtime_container, ssh_server, "Starts")
    Rel(runtime_container, tui_server, "Requests TUI", "HTTP")

    %% Container infrastructure
    Rel(container_engine, docker, "Executes", "CLI/API")
    Rel(provision, container_engine, "Builds")

    %% Layout adjustments for overlapping labels
    ...
    UpdateRelStyle(runtime_container, provision, $offsetX="-30")
    UpdateRelStyle(runtime_container, ssh_server, $offsetX="30")
```
...
## Appendix: Full Diagram (Original)

This is the original unoptimized diagram showing all containers in a single boundary. It's preserved for reference but may have overlapping labels in some renderers.

```mermaid
C4Container
    title Container Diagram - Invowk (Full Detail)
