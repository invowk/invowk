...
| **Optional LLM Providers** | LLM-powered `invowk audit` analysis plus `invowk agent cmd` and `invowk agent mod` create/change authoring via Claude, Codex/OpenAI, Gemini, or local Ollama. | OpenAI-compatible HTTP APIs / local CLIs |
...
1. **Multi-Runtime Support**: Invowk doesn't mandate a single execution environment. Users choose native shell for speed, virtual-sh for portable shell scripts, virtual-lua for embedded Lua scripts, or containers for isolation.
