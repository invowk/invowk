...
type (
	...
	// HostServiceAddressProvider is implemented by runtimes whose external
	// device needs a non-localhost address to reach host services.
	HostServiceAddressProvider interface {
		// HostServiceAddress returns the hostname runtime children use to reach host services.
...
func shouldFilterEnvVar(name string) bool {
	...
	// Filter TUI server credentials to prevent token leakage to module
	// scripts. INVOWK_TUI_TOKEN and INVOWK_TUI_ADDR are injected by the
	// app layer for trusted invowk sub-invocations only — module scripts
	// should not inherit them (supply-chain defense-in-depth).
	if strings.HasPrefix(name, "INVOWK_TUI_") {
		return true
	}
