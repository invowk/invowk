...
- Environment Pollution: Look for modules that export sensitive variables or write to shared host paths (e.g., ~/.ssh, /tmp, project-root .env) to attack the project importing them.
- Silent Exfiltration: Look for commands that transmit data to remote hosts during phases where network access is unexpected (e.g., 'lint', 'format', 'test').
- Latent Injection: Look for scripts that write content to files that will be executed, sourced, or compiled later by the project.
- Recursive Invowk Calls: Identify scripts that call `invowk` again with untrusted or unsanitized arguments (similar to `eval`).
...
- Include a precise recommendation that changes behavior, not just "review this". Provide specific code-level fixes (e.g., "Use double quotes for variable expansion" or "Use a specific sha256 hash instead of 'latest' tag").
- Use line when the suspicious behavior maps to a specific line. Use 0 only when no reliable line exists. Treat the first line of the provided script content as line 1.
...
Identify any opaque data strings (Base64, Hex) and attempt to decode them to verify they don't contain malicious commands or exfiltration endpoints.
