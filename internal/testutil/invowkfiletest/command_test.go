...
func TestNewTestCommand_Defaults(t *testing.T) {
	...
	if impl.Runtimes[0].Name != invowkfile.RuntimeNative {
...
func TestNewTestCommand_WithRuntimeConfig(t *testing.T) {
	...
	if gotRC.Name != invowkfile.RuntimeContainer {
		t.Errorf("Runtime.Name = %v, want %v", gotRC.Name, invowkfile.RuntimeContainer)
	}
...
func TestNewTestCommand_WithPlatform(t *testing.T) {
	...
	if cmd.Implementations[0].Platforms[0].Name != invowkfile.PlatformLinux {
		t.Errorf("Platform = %v, want %v", cmd.Implementations[0].Platforms[0].Name, invowkfile.PlatformLinux)
	}
}
...
func TestNewTestCommand_WithPlatforms(t *testing.T) {
	...
	if cmd.Implementations[0].Platforms[0].Name != invowkfile.PlatformLinux {
		t.Errorf("Platforms[0] = %v, want %v", cmd.Implementations[0].Platforms[0].Name, invowkfile.PlatformLinux)
	}

	if cmd.Implementations[0].Platforms[1].Name != invowkfile.PlatformMac {
		t.Errorf("Platforms[1] = %v, want %v", cmd.Implementations[0].Platforms[1].Name, invowkfile.PlatformMac)
	}
}
...
func TestNewTestCommand_WithFlag(t *testing.T) {
	cmd := NewTestCommand("deploy",
		WithFlag("env",
			...
			FlagType(invowkfile.FlagTypeString),
	...
	if flag.Type != invowkfile.FlagTypeString {
		t.Errorf("Flag.Type = %v, want %v", flag.Type, invowkfile.FlagTypeString)
	}
...
func TestNewTestCommand_WithArg(t *testing.T) {
	cmd := NewTestCommand("copy",
		WithArg("source",
			...
			ArgType(invowkfile.ArgumentTypeString),
			ArgValidation("^[a-z]+$"),
		),
		WithArg("dest",
			ArgDefault("/tmp"),
	...
	if arg0.Type != invowkfile.ArgumentTypeString {
		t.Errorf("Args[0].Type = %v, want %v", arg0.Type, invowkfile.ArgumentTypeString)
	}
...
func TestNewTestCommand_CombinedOptions(t *testing.T) {
	cmd := NewTestCommand("deploy",
		...
		WithPlatform(invowkfile.PlatformLinux),
