...
2. Classify each learning:
   - `AGENTS.md`: repository-wide governance, indexes, or routing.
   ...
   - Compatibility surfaces: update `.claude/*` only when the canonical
     `AGENTS.md` or `.agents/*` source explicitly requires its compatibility
     copy or symlink to stay synchronized. Never cite `.claude/*` as canonical.
