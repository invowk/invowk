...
- PR benchmark alerts fail by default through `--error-on-alert`. If a maintainer has deliberately accepted an expected feature cost, add the `benchmarks: accepted-regression` PR label; workflows set `BENCHER_ERROR_ON_ALERT=false` for that PR, so alerts remain visible in Bencher while the check becomes advisory. Push and manual benchmark runs are also advisory: they publish main/release history and GitHub checks without making already-merged alert history fail CI. Do not use advisory mode for benchmark script bugs, missing thresholds, or unexplained regressions.
