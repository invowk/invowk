...
- **Runtime.** The first run took 28 min on one local worker, including a ~90 s pre-flight (results in `formal/README.md`). The manual workflow job allows 90 min for this target set; if a run outgrows it, raise the budget and open the deferred sharding follow-up rather than adding `-short`.
