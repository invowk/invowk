...
// AddRequirement adds a module requirement to the requires block in invkmod.cue.
...
func AddRequirement(invkmodPath string, req ModuleRef) error {
	data, err := os.ReadFile(invkmodPath)
	if err != nil {
		return fmt.Errorf("failed to read %s: %w", invkmodPath, err)
	}
	...
	return atomicWriteFile(invkmodPath, []byte(strings.Join(lines, "\n")))
}

// RemoveRequirement removes a module requirement matching gitURL and subPath
// from the requires block in invkmod.cue.
...
func RemoveRequirement(invkmodPath, gitURL, subPath string) error {
	data, err := os.ReadFile(invkmodPath)
	if err != nil {
