...
- Initial committed baselines were later populated from reviewed advisory full-scan reports and tightened with focused survivor-killing tests. The current committed baselines accept 1,631 root-module survivors and 925 `tools/goplint` survivors. Rollout CI remains advisory while maintainers reduce the baseline through focused tests and decide when blocking mode is stable enough. The first real blocking all-module run on 2026-06-01 confirmed that a blanket empty-baseline full scan is too broad for initial adoption, so the root full manifest now starts from a smaller baselineable seed.
...
- A corrected `tools/goplint` full run on 2026-06-02 normalized file-target mutant IDs from `./goplint/...` to `goplint/...`, restoring stable single-mutant rerun support for goplint escaped IDs.
