...
- **Coverage parser fix (task 4.8).** TLC appends the location ` (l c l c)` to the coverage line of a LET-headed action or an unnamed sub-action, for example `<Next line 114, col 1 to line 114, col 4 of module LockIntegrity (115 8 115 21)>: 0:504`, and the old pattern skipped those lines. The only snapshot difference from the baseline is `Next` now listed as zero-coverage in the two LockIntegrity witness commands above; all 97 verdicts, violated properties, state counts, trace verdicts, and golden digests are identical, and no passing command gains an uncovered action.
- **Verification environment.** `make test`: 8516 non-CLI tests pass; its `test-cli` step needs `-race`, which needs cgo, and this machine has no C compiler, so the CLI suite was run as `go test ./tests/cli/...` without `-race` (passes). `make test-cli` and native mirrors are otherwise unaffected: no txtar files change. `make sonar-local` ran as a pre-commit hook (0 unresolved issues; no branch analysis yet). `/learn` is left to the orchestrator.
