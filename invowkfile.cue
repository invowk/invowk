...
//
// Note: Module metadata (module, version, description, requires) belongs in invowkmod.cue.
...
// Root-level env has the lowest priority from invowkfile configuration.
...
cmds: [
	...
	// Root-level (lowest priority) -> Command-level -> Implementation-level (highest priority from invowkfile)
	...
	// - Separating sensitive values from invowkfile definitions
	...
	//   CLI flag (--workdir) > Implementation > Command > Root > Default (invowkfile dir)
	//
	// Paths should use forward slashes (/) for cross-platform compatibility.
	// Relative paths are resolved from the invowkfile directory.
	...
	// Relative paths are resolved from the invowkfile directory
	{
		...
		description: "Relative workdir resolved from invowkfile location"
		workdir:     "examples"
		implementations: [
			{
				script: """
					...
					echo "invowkfile directory location."
					echo ""
	...
	// to ALL commands in the invowkfile. This is useful for:
	...
	//
	// Note: This invowkfile has root-level depends_on: tools: [{alternatives: ["sh", "bash"]}]
		...
		implementations: [
			{
				script: """
					...
					echo ""
					echo "  1. ROOT LEVEL (from invowkfile):"
					echo "     - tools: [sh, bash]"
					echo ""
		...
		implementations: [
			{
				script: """
					...
					echo "=========================================="
					echo ""
					echo "Imagine an invowkfile with root-level depends_on:"
					echo ""
					echo "  depends_on: {"
					echo "    env_vars: ["
