...
- [X] T033 [P] Add SPDX MPL-2.0 license headers to all new Go files: internal/selfupdate/github.go, internal/selfupdate/detect.go, internal/selfupdate/checksum.go, internal/selfupdate/selfupdate.go, and cmd/invowk/upgrade.go
- [X] T034 Create testscript (txtar) CLI integration tests for `invowk upgrade --check` and error paths in tests/cli/ (note: may require helper binary or env var for GitHub API mocking in testscript context)
- [X] T035 Verify `invowk upgrade --help` output matches CLI contract specification in specs/009-install-and-upgrade/contracts/cli-upgrade.md
