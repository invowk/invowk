...
.goreleaser.yaml                    # ldflags path update + configure homebrew_casks section
...
#   └── Casks/invowk.rb             # Auto-generated by GoReleaser

# GoReleaser config (Phase 5)
.goreleaser.yaml                    # Configure homebrew_casks section
...
**Scope**: Enable Homebrew distribution via GoReleaser's `homebrew_casks` integration.

**Steps**:
1. Create `invowk/homebrew-tap` GitHub repository
2. Add `HOMEBREW_TAP_TOKEN` secret to main repo
3. Configure `homebrew_casks:` section in `.goreleaser.yaml`
