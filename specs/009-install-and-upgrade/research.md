...
**Decision**: Use GoReleaser's built-in `homebrew_casks` configuration (migrated from the deprecated `brews` key per GoReleaser v2.10+).

**Rationale**: GoReleaser automates the entire Homebrew cask lifecycle — generating the cask file, computing checksums, and pushing to the tap repository. This eliminates manual cask maintenance and ensures the tap is always in sync with releases.
...
3. **GoReleaser config**: Configure `homebrew_casks:` section (migrated from deprecated `brews:`)
4. **Cask features**: Binary distribution with quarantine removal hook on macOS, multi-platform support
